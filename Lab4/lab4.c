...
int objectNumber = 4;        // Which object to draw (1 ,2, 3, 4, 5, or 6)?
...
void draw() {
    ...
    // int v = dodecTriangles[i][j];  // Vertex number for vertex j of face i.
    glDisable(GL_LIGHTING);
    glBegin(GL_LINE_LOOP);
    int i;
    for(i=0;i<60;i++){
        int index[] = {dodecTriangles[i][0],dodecTriangles[i][1],dodecTriangles[i][2]};
        glVertex3d(dodecVertices[index[0]][0],dodecVertices[index[0]][1],dodecVertices[index[0]][2]);
        glVertex3d(dodecVertices[index[1]][0],dodecVertices[index[1]][1],dodecVertices[index[1]][2]);
        glVertex3d(dodecVertices[index[2]][0],dodecVertices[index[2]][1],dodecVertices[index[2]][2]);
    }
